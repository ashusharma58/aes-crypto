...
function deriveKey (masterKey, saltBuffer, keyLength, options) {
  ...
  switch (kdf) {
    case KDF.PBKDF2:
      derivedKeyBuffer = crypto.pbkdf2Sync(masterKey, saltBuffer, kdfIterations, keyLength, kdfDigest)
      break

    default:
      throw new CryptoError(null, SOURCE, `Cannot Derive Key with KDF '${kdf}'`)
  }
...
function extractKeyFromToken (token = '', keyLength = 32) {
  ...
  const derivedKeyBuffer = crypto.pbkdf2Sync(checksum, saltBuffer, kdfIterations, keyLength, kdfDigest)
