...
function encrypt(algorithm) {
  ...
  try {
    ...
    var payloadObj = payloadFunc(options, cipherTextObj, ivObj, saltObj);
    var {
...
function generateEncryptPayloadWithKDF(options, cipherTextObj, ivObj, saltObj) {
  ...
  var payloadBuffer = Buffer.concat([saltBuffer, ivBuffer, cipherTextBuffer]);
  var payloadString = dataSeparator ? [saltString, ivString, cipherTextString].join(dataSeparator) : payloadBuffer.toString(cipherTextFormat);
  return {
...
function generateEncryptPayloadWithoutKDF(options, cipherTextObj, ivObj) {
  ...
  var payloadBuffer = Buffer.concat([ivBuffer, cipherTextBuffer]);
  var payloadString = dataSeparator ? [ivString, cipherTextString].join(dataSeparator) : payloadBuffer.toString(cipherTextFormat);
  return {
    buffer: payloadBuffer,
    string: payloadString
  };
...
function getPayloadPartsWithKDF(source, params, options) {
  ...
  if (dataSeparator) {
    var [saltString, ivString, cipherTextString] = payload.split(dataSeparator);
...
function getPayloadPartsWithoutKDF(source, params, options) {
  ...
  if (dataSeparator) {
    var [ivString, cipherTextString] = payload.split(dataSeparator);
